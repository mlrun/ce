...
| Kafka | `kafka.enabled` | Strimzi CRDs — `strimzi-kafka-operator` sub-chart must also be enabled as a prerequisite; it ships the CRDs in its `crds/` dir, which Helm installs before templates, so the `Kafka`/`KafkaNodePool` CRs are plain release-managed manifests (no hooks). `helm upgrade` patches them in place, preserving the KRaft cluster id. PVC retention is controlled by `kafka.storage.deleteClaim` (default `false`). | `templates/kafka/` |
...
5. CRD dependencies — if a sub-chart ships its CRDs in a `crds/` directory (as `strimzi-kafka-operator` does), Helm installs them before any templates, so your CRs can be plain release-managed manifests. Only reach for `helm.sh/hook: post-install`/`post-upgrade` + `hook-weight` when a sub-chart installs its CRDs via *templates* (an in-release ordering problem). Prefer normal manifests when possible — hooks are not release-managed, so `helm upgrade` deletes and recreates them, which for stateful CRs (e.g. Kafka's KRaft id on a retained PVC) can corrupt identity.
